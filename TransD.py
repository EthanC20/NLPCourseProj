...
class TransD(nn.Module):
    def __init__(self, entity_num, relation_num, norm=1, dim=100):
        ...
        self.entity_mappings = self._init_emb(entity_num)
        self.relation_mappings = self._init_emb(relation_num)
    ...
    def _projection(self, emb_e, emb_m, proj_vec):
        # [b, k] + sigma ([b, k] * [b, k]) * [b, k]
        return emb_e + torch.sum(emb_e * emb_m, dim=-1, keepdim=True) * proj_vec
    ...
    def _distance(self, triplets):
        ...
        heads_proj = self._projection(heads, self.entity_mappings(triplets[:, 0]), relations)
        tails_proj = self._projection(tails, self.entity_mappings(triplets[:, 2]), relations)
        return (heads_proj + relations - tails_proj).norm(p=self.norm, dim=1)

    # The rest of the methods (link_predict and evaluate) remain unchanged.
...
if __name__ == '__main__':
    ...
    best_mrr = 0
    learning_rate = 0.0001  # 学习率建议粗调0.01-0.001，精调0.001-0.0001
    ...
    # 构建模型
    model = TransD(len(ent2id), len(rel2id), norm=distance_norm, dim=embedding_dim).cuda()
    # model.load_state_dict(torch.load('transD_best.pth'))
    ...
    for epoch in range(epochs):
        ...
        for i, (local_heads, local_relations, local_tails) in enumerate(train_data_loader):
            ...
            # pd要尽可能小， nd要尽可能大
            loss = criterion(pd, nd, torch.tensor([-1], dtype=torch.float).cuda())
            loss.backward()
            all_loss += loss.data
            optimizer.step()
        ...
        if validation and (epoch+1) % dev_interval == 0:
            ...
            if mrr >= best_mrr:
                best_mrr = mrr
                improve = '*'
                torch.save(model.state_dict(), 'transD_best.pth')
            torch.save(model.state_dict(), 'transD_latest.pth')
